...
def load_credentials_from_toml(toml_file_path):
    try:
        secrets = toml.load(toml_file_path)
        google_cloud_credentials = secrets.get('google_cloud', {})

        # Extract necessary fields for service account credentials
        credentials_info = {
            "type": google_cloud_credentials.get('type'),
            "project_id": google_cloud_credentials.get('project_id'),
            "private_key_id": google_cloud_credentials.get('private_key_id'),
            "private_key": google_cloud_credentials.get('private_key').replace("\\n", "\n"),  # Make sure newlines are correct
            "client_email": google_cloud_credentials.get('client_email'),
            "client_id": google_cloud_credentials.get('client_id'),
            "auth_uri": google_cloud_credentials.get('auth_uri'),
            "token_uri": google_cloud_credentials.get('token_uri'),
            "auth_provider_x509_cert_url": google_cloud_credentials.get('auth_provider_x509_cert_url'),
            "client_x509_cert_url": google_cloud_credentials.get('client_x509_cert_url'),
            "universe_domain": google_cloud_credentials.get('universe_domain')
        }

        # Create credentials from the information
        credentials_object = service_account.Credentials.from_service_account_info(credentials_info)
        return credentials_object
    except Exception as e:
        print(f"Error loading credentials from TOML: {str(e)}")
        raise e
